        ...
        try:
            ...
            try:
                cur.execute(
                    ...
                    ) TABLESPACE {Config.ORACLE_VECTOR_TABLESPACE}
                    """
                )
            except oracledb.DatabaseError as exc2:  # pragma: no cover - depends on DB state
                err2 = exc2.args[0]
                if getattr(err2, "code", None) == 43853:  # ORA-43853
                    raise RuntimeError(
                        "VECTOR type requires an ASSM tablespace; set ORACLE_VECTOR_TABLESPACE"
                    ) from exc2
                raise
